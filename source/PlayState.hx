...
class PlayState extends MusicBeatState
{
	...
	{
		if (SONG.song.toLowerCase() == 'parasite' && isStoryMode)
		{
			agotiCrazy();
		}
		else
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			else
			{
				...
				if(dad.animation.curAnim.name.startsWith('sing'))
					temp = dad.animation.curAnim.name;
					if(dad.animation.finished && temp == dad.animation.curAnim.name)
						if (SONG.song.toLowerCase() == 'parasite' && curStep >= 550 && curStep <= 561)
						{
							trace('fuck you, no idle');
						}
						else
						{
							dad.dance();
