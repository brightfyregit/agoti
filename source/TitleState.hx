...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('titleBG', 'preload'));
		bg.screenCenter();
		bg.updateHitbox();
		...
		gfDance = new FlxSprite(0, 0).loadGraphic(Paths.image('agoti', 'preload'));
		gfDance.setGraphicSize(Std.int(gfDance.width * 0.4));
	...
	override function beatHit()
	{
		...
		{
			case 1:
				createCoolText(['agoti', 'sugarratio', 'brightfyre', 'kullix']);
				failedToPrint = false;
			case 3:
				if (failedToPrint)
				{
